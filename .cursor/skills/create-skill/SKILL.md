...
- `scripts/` — only when the skill needs executable helpers. Scripts are `.sh` (POSIX shell), executable, and live under `scripts/` — never inline script bodies in `SKILL.md`. Reference them from `SKILL.md` with a relative link (`scripts/foo.sh`) and the run command; keep script behavior and exit codes documented in the script header.
...
- Any script under `.cursor/` must be a `.sh` POSIX shell script — no `.js`/`.ts`/`.py` (per `management/general.mdc`).

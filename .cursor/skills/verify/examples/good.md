...
```sh
python3 .cursor/skills/verify/scripts/verify.py src/4_modules/user/api/user.ts
pnpm exec vitest run src/4_modules/user/api/user.test.ts --config vitest.unit.browser.config.ts
```
...
> Done: wrapped `useGetUser` in `~modules/user/api`. Static checks are green. I did **not** run the integration tests — propose `pnpm run check:tests:int:tsx` if you want them.
